...
const jsonResponse = createJsonResponder(cacheControl.swr(120, 300));

const USERNAME = "maxeisen";
const HEATMAP_WEEKS = 10;

// Absorb bursts that slip past the CDN's 2-min edge cache (cold cache, or
// local dev with no edge). Shorter than the edge max-age so it never serves
// data older than the CDN would.
...
// Fetch + shape the dashboard payload. Returns { latest, contributions };
// throws on a network error so the handler can map it to a 502. A failed
// GraphQL call (but successful request) degrades to an empty heatmap rather
// than throwing, matching the prior behaviour.
...
export default async function handler() {
	...
	try {
		const payload = await memo("latest", () => fetchGithub(token));
